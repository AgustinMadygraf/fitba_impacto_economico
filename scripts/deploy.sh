...
TIMESTAMP=$(date +%s)
$SSH_CMD "cd $REMOTE_DIR &&           echo '[REMOTE] Fetching...' && git fetch origin &&           echo '[REMOTE] Resetting...' && git reset --hard origin/main &&           echo '[REMOTE] Actualizando versión JS (Cache Busting)...' &&           sed -i 's/v=__TIMESTAMP__/v=$TIMESTAMP/g' frontend/index.html &&           echo '[REMOTE] Instalando dependencias...' && cd backend && ../.venv/bin/pip install -r requirements.txt && cd .. &&           echo '[REMOTE] Reiniciando servicio...' && curl -s https://fitba.datamaq.com.ar/api/config -o $REMOTE_DIR/backend/data/remote_params.json || true && sudo systemctl restart fitba-backend && sleep 5 && echo "[REMOTE] Verificando API de configuración..." && STATUS=$(curl -o /dev/null -s -w "%{http_code}" https://fitba.datamaq.com.ar/api/config) && if [ "$STATUS" -eq 500 ]; then echo "[REMOTE] [WARNING] La API de configuración devolvió error 500"; else echo "[REMOTE] API de configuración devolvió status $STATUS"; fi"
